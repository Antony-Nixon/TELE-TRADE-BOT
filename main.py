...
from get_price import PriceManager
...
# Start tracking common cryptocurrencies
price_manager.start(["BTCUSDT", "ETHUSDT", "BNBUSDT", "DOGEUSDT", "SOLUSDT"])

def fetch_crypto_price(token: str) -> str:
    """Fetch the current price of a cryptocurrency"""
    price = price_manager.get_price(token)
    if price is None:
        return f"Sorry, could not fetch the price for {token}. Make sure you entered a valid token symbol."
    return f"The current price of {token} is ${price}"
...
if __name__ == "__main__":
    try:
        ...
        # Ensure price manager is properly stopped when the bot exits
        price_manager.stop()